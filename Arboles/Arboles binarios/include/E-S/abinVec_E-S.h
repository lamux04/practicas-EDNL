#ifndef AbinVec_E_S_H
#define AbinVec_E_S_H
...
template <typename T>
void rellenarAbin(AbinVec<T>& A, const T& fin)
// Pre: A está vacío.
...
template <typename T>
void rellenarDescendientes(typename AbinVec<T>::nodo n, AbinVec<T>& A, const T& fin)
...
//       usando fin para introducir nodos nulos.
{
   ...
   assert(A.hijoIzqdo(n) == AbinVec<T>::NODO_NULO &&
          A.hijoDrcho(n) == AbinVec<T>::NODO_NULO);
...
template <typename T>
void rellenarAbin(istream& is, AbinVec<T>& A)
// Pre: A está vacío.
...
template <typename T>
void rellenarDescendientes(istream& is, typename AbinVec<T>::nodo n, AbinVec<T>& A, const T& fin)
// Pre: n es un nodo hoja de A.
...
template <typename T>
void imprimirAbin (const AbinVec<T>& A)
...
template <typename T>
void imprimirDescendientes (typename AbinVec<T>::nodo n, const AbinVec<T>& A)
// Post: Muestra los descendientes de n en la salida estándar.
{
   if (A.hijoIzqdo(n) != AbinVec<T>::NODO_NULO)
   {
      cout << "Hijo izqdo de " << A.elemento(n) << ": "
                << A.elemento(A.hijoIzqdo(n)) << endl;
      imprimirDescendientes(A.hijoIzqdo(n), A);
   }
   if (A.hijoDrcho(n) != AbinVec<T>::NODO_NULO)
   {
...
template <typename T>
void imprimirAbin (ostream& os, const AbinVec<T>& A, const T& fin)
...
template <typename T>
void imprimirDescendientes (ostream& os, typename AbinVec<T>::nodo n, const AbinVec<T>& A, const T& fin)
