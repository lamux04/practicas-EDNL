...
template <typename T>
void rellenarAgen(AgenEnla<T>& A, const T& fin)
// Pre: A est� vac�o.
...
template <typename T>
void rellenarDescendientes(typename AgenEnla<T>::nodo n, AgenEnla<T>& A, const T& fin)
...
//       usando fin para introducir nodos nulos.
{
   ...
   assert(A.hijoIzqdo(n) == AgenEnla<T>::NODO_NULO);
...
template <typename T>
void rellenarAgen(istream& is, AgenEnla<T>& A)
// Pre: A est� vac�o.
...
template <typename T>
void rellenarDescendientes(istream& is, typename AgenEnla<T>::nodo n, AgenEnla<T>& A, const T& fin)
// Pre: n es un nodo hoja de A.
...
template <typename T>
void imprimirAgen(const AgenEnla<T>& A)
...
template <typename T>
void imprimirDescendientes(typename AgenEnla<T>::nodo n, const AgenEnla<T>& A)
// Post: Muestra los descendientes de n en la salida est�ndar.
{
   if (A.hijoIzqdo(n) != AgenEnla<T>::NODO_NULO)
   {
      typename AgenEnla<T>::nodo hijo = A.hijoIzqdo(n);
      cout << "Hijo izqdo de " << A.elemento(n) << ": "
      ...
      while (hijo != AgenEnla<T>::NODO_NULO) {
         ...
         if (A.hermDrcho(hijo) != AgenEnla<T>::NODO_NULO)
...
template <typename T>
void imprimirAgen(ostream& os, const AgenEnla<T>& A, const T& fin)
...
template <typename T>
void imprimirDescendientes(ostream& os, typename AgenEnla<T>::nodo n, const AgenEnla<T>& A, const T& fin)
